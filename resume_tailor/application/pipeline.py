...
from resume_tailor.backend.jobs.apify_job import invoke_apify_linkedin_retrieval
...
from resume_tailor.backend.utils.clipboard import read_clipboard
...
from resume_tailor.backend.jobs.job_requirements import build_job_requirement_catalog, job_description_sha256
from resume_tailor.backend.jobs.job_text import validate_confirmed_job_description
...
def _analysis_dependency_versions(
    ...
    if provider == "gemma_local":
        from resume_tailor.backend.providers.gemma_analysis import resolve_gemma_analysis_model
        from resume_tailor.backend.providers.ollama_transport import ollama_dependency_versions
...
def _required_pdf_text(
    ...
    *,
    ...
    if document_format == "headless":
        from resume_tailor.backend.documents.headless_render import HEADLESS_SECTION_HEADINGS as headings
    else:
        from resume_tailor.backend.documents.docx_extract import SECTION_HEADINGS as headings
...
def run_pipeline(
    ...
    *,
    ...
    with cancellable_commands(active_hooks.cancel_event):
        run_directory = _run_pipeline(request, active_hooks)
    return PipelineResult(run_directory=run_directory)


def _run_pipeline(request: PipelineRequest, hooks: PipelineHooks) -> Path:
    hooks.progress("validating_input", "Validating the résumé and job input.")
    resume_path = _validate_resume_path(request.resume)
    timeout_seconds, antigravity_duration = request.timeout
    ...
    antigravity_strength = request.antigravity_strength
    grok_model = request.grok_model
    grok_strength = request.grok_strength
    codex_model = request.codex_model
    codex_strength = request.codex_strength
    retry_context = request.retry_context
    retry_inputs = None
    antigravity_retry_context = request.antigravity_retry_context
    antigravity_reprocess_context = request.antigravity_reprocess_context
    ...
    if retry_context is not None:
        ...
        if request.job_url is not None or request.clipboard or request.job_file is not None:
    ...
    if antigravity_retry_inputs is not None:
        ...
        company = validate_label(retry_inputs.context.company, "Stored company")
        role = validate_label(retry_inputs.context.role, "Stored role")
        job_description = retry_inputs.job_description
        job_source = "source-evidence-retry"
    elif request.job_url is not None:
        requested_linkedin_url = validate_linkedin_url(request.job_url)
        company: str | None = None
        role: str | None = None
        job_description: str | None = None
        job_source = "linkedin-url"
    else:
        assert request.company is not None and request.role is not None
        company = validate_label(request.company, "Company")
        role = validate_label(request.role, "Role")
        if request.clipboard:
            job_description, job_source = read_clipboard()
        else:
            assert request.job_file is not None
            job_description = read_text_file(
                request.job_file.expanduser(),
                label="job description",
            )
            job_source = request.job_source_override
    ...
    try:
        analytics_path = request.analytics_db
    ...
    try:
        ...
        if requested_linkedin_url is not None:
            ...
            hooks.present("linkedin_posting", fetched_job)
            ...
            company = validate_label(fetched_job["company"], "Extracted company")
            role = validate_label(fetched_job["job_title"], "Extracted job title")
        ...
        if antigravity_retry_inputs is not None:
            ...
            if analysis_provider == "codex":
                ...
                from resume_tailor.backend.providers.grok_analysis import prepare_grok_analysis_schema
                ...
                from resume_tailor.backend.providers.gemma_analysis import prepare_gemma_analysis_schema
            ...
            if not analysis.get("recommended_edits") and analysis.get("discarded_no_op_edit_ids") and metadata["stage"] != "resuming":
                hooks.present(
                    ...
                    {
                        "message": (
                            "The analysis found no evidence-supported résumé changes."
                        )
                    },
                )
            ...
            metadata["codex_analysis_approval"] = (
                record_codex_analysis_approval(
                    ...
                    role=role,
                    approval_mode=(
                        "assume_yes" if request.yes else "interactive"
                    ),
                )
            )
            _update_metadata(metadata, metadata_path, run_directory=run_directory)
        ...
        _update_metadata(metadata, metadata_path, run_directory=run_directory)
        hooks.present("content_diff", {"content_diff": content_diff})
        ...
        metadata["revision_cycle"]["initial"]["content_approval"] = {
            "decision": "approved",
            "mode": "assume_yes" if request.yes else "explicit",
        ...
        if document_format == "headless":
            from resume_tailor.backend.documents.headless_render import render_headless_docx
        ...
        # -----------------------------------------------------------------
        preselected = request.initial_qa_provider
        ...
        while True:
            ...
            authorization_record = {
                ...
                "timestamp": utc_now_iso(),
                "yes_flag_did_not_authorize": bool(request.yes),
            }
